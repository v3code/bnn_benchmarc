...
def get_dataset(config: ConfigDict):
    ...
    if dataset == 'mnist':
        ...
        train_dataset = Omniglot(background=True, transform=config.dataset_train_transforms, download=True,
                                 root=train_root)
        val_dataset = Omniglot(transform=config.dataset_val_transforms, download=True,
                               root=val_root)
